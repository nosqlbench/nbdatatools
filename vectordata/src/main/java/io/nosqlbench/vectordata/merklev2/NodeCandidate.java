...
/// 
/// This record encapsulates all the information needed to evaluate
...
/// @param isLeafNode Whether this is a leaf node or internal node
/// @param coveredRequiredChunks Set of required chunks this node would cover
/// @param coveredAllChunks Set of all chunks this node would download (including prefetch)
/// @param estimatedBytes Estimated download size for this node
/// @param efficiency Efficiency ratio (required chunks / all chunks)
/// @param explanation Human-readable explanation of this candidate
///
public record NodeCandidate(
    int nodeIndex,
    boolean isLeafNode,
    Set<Integer> coveredRequiredChunks,
    Set<Integer> coveredAllChunks,
    long estimatedBytes,
    double efficiency,
    String explanation
) {
    ...
    ///
    /// Creates a NodeCandidate with validation.
    ///
    /// @param nodeIndex The merkle tree node index (must be non-negative)
    /// @param isLeafNode Whether this is a leaf node
    /// @param coveredRequiredChunks Required chunks covered (must not be null)
    /// @param coveredAllChunks All chunks covered (must not be null)
    /// @param estimatedBytes Estimated download size (must be non-negative)
    /// @param efficiency Efficiency ratio (must be between 0.0 and 1.0)
    /// @param explanation Human-readable explanation (must not be null or empty)
    ...
    public NodeCandidate {
        ...
        coveredRequiredChunks = Set.copyOf(coveredRequiredChunks);
        coveredAllChunks = Set.copyOf(coveredAllChunks);
    }
