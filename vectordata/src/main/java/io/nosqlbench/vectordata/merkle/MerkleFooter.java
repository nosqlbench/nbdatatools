...
public record MerkleFooter(long chunkSize, long totalSize, int bitSetSize, byte footerLength) {

  /**
   The fixed size of the footer in bytes
   8 bytes for chunkSize + 8 bytes for totalSize + 4 bytes for bitSetSize + 1 byte for footerLength
   */
  public static final int FIXED_FOOTER_SIZE = Long.BYTES * 2 + Integer.BYTES + Byte.BYTES;
   ...
   */
  public static MerkleFooter create(long chunkSize, long totalSize, int bitSetSize) {
    // Calculate the footer length: fixed size
    byte footerLength = (byte) (FIXED_FOOTER_SIZE);
    return new MerkleFooter(chunkSize, totalSize, bitSetSize, footerLength);
  }

  /**
   Creates a new MerkleFooter with the given parameters and calculates the footer length.
   BitSet size is set to a default value of 1 byte to ensure the full bitset is persisted.
   @param chunkSize
   the chunk size in bytes
   @param totalSize
   the total size of the data in bytes
  ...
  public static MerkleFooter create(long chunkSize, long totalSize) {
    // Use a default bitSetSize of 1 byte to ensure the full bitset is persisted
    return create(chunkSize, totalSize, 1);
  }
  ...
  public static MerkleFooter fromByteBuffer(ByteBuffer buffer) {
    // Expect a full footer: chunkSize (8) + totalSize (8) + bitSetSize (4) + footerLength (1)
    ...
    if (duplicate.remaining() < expected) {
      // If we don't have enough bytes, try to find the footer length byte at the end
      if (duplicate.remaining() > 0) {
        byte footerLength = duplicate.get(duplicate.limit() - 1);

        // If the footer length is valid and matches the buffer size, use it
        if (footerLength > 0 && footerLength <= duplicate.remaining()) {
          // Position the buffer at the start of the footer based on the footer length
          duplicate.position(duplicate.limit() - footerLength);

          // Try to read the available fields
          long chunkSize = 0;
          long totalSize = 0;
          int bitSetSize = 0;

          // Read as many fields as we can
          if (duplicate.remaining() >= 8) {
            chunkSize = duplicate.getLong();
          }
          if (duplicate.remaining() >= 8) {
            totalSize = duplicate.getLong();
          }
          if (duplicate.remaining() >= 4) {
            bitSetSize = duplicate.getInt();
          }

          // Skip to the footer length byte
          if (duplicate.remaining() > 1) {
            duplicate.position(duplicate.limit() - 1);
          }
      ...
      // If we couldn't find a valid footer length, throw an exception
      throw new IllegalArgumentException(
          "Invalid Merkle footer buffer size: " + buffer.remaining()
          + ", expected at least " + expected);
    }

    // If we have enough bytes, read the full footer
    long chunkSize = duplicate.getLong();
    long totalSize = duplicate.getLong();
  ...
  public static MerkleFooter fromRemoteUrl(URL url) throws IOException {
    ...
    try {
      ...
      throw new RuntimeException(
          "Error while reading footer from URL " + url.toString() + ": " + e.getMessage(),
          e
      );
    }
