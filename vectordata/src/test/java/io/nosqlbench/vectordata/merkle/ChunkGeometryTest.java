...
public class ChunkGeometryTest {
    ...
    public void testContentSizeValidation() {
        ...
        assertThrows(IllegalArgumentException.class, () -> {
            new ChunkGeometryDescriptor(-1); // Negative content size
        });

        // Test valid content sizes
        assertDoesNotThrow(() -> {
            new ChunkGeometryDescriptor(0); // Zero content size is valid (empty file)
            new ChunkGeometryDescriptor(1024); // Small content size
            new ChunkGeometryDescriptor(1024 * 1024); // 1MB
            new ChunkGeometryDescriptor(1024 * 1024 * 1024); // 1GB
        });

        // Test that chunk size is always a power of 2
        ChunkGeometryDescriptor geometry = new ChunkGeometryDescriptor(1000);
