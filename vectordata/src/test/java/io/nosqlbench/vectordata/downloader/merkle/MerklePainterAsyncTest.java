    ...
    void testPaintAsync(@TempDir Path tempDir) throws Exception {
        ...
        try {
            ...
            for (int i = startChunk; i <= endChunk; i++) {
                boolean isIntact = pane.isChunkIntact(i);
                System.out.println("[DEBUG_LOG] Chunk " + i + " isIntact: " + isIntact);
                if (!isIntact) {
                    System.out.println("[DEBUG_LOG] Chunk " + i + " is NOT intact - checking ShadowTree state");
                }
                assertTrue(isIntact, "Chunk " + i + " should be intact");
