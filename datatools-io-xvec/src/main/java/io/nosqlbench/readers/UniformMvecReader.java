...
/// A SizedReader implementation for uniform mvec files (minivec, f16 half-precision float vectors).
///
/// The mvec format consists of:
...
@FileExtension({".mvec", ".mvecs"})
public class UniformMvecReader extends ImmutableSizedReader<float[]> implements VectorFileArray<float[]> {
    ...
    /// Constructs a new UniformMvecReader.
    public UniformMvecReader() {
    }

    /// Opens the mvec file at the given path for reading.
    ///
    /// @param filePath The path to the mvec file
    ...
    public List<float[]> subList(int fromIndex, int toIndex) {
        ...
        return new SublistMvecReader(this, fromIndex, toIndex);
    }
    ...
    /// A view of a sublist of an MvecReader.
    private static class SublistMvecReader extends ImmutableSizedReader<float[]> {
        private final UniformMvecReader parent;
        private final int offset;
        private final int size;

        public SublistMvecReader(UniformMvecReader parent, int fromIndex, int toIndex) {
            this.parent = parent;
