...
/// A SizedReader implementation for uniform svec files (16-bit short integer vectors).
///
...
public class UniformSvecReader extends ImmutableSizedReader<short[]> implements VectorFileArray<short[]> {
    ...
    /// Creates a new UniformSvecReader.
    public UniformSvecReader() {
    }

    /// Opens the svec file at the given path for reading.
    ///
    ...
    public short[] get(int index) {
        ...
        try {
            ...
            buffer.asShortBuffer().get(vector);
    ...
    public int indexOf(Object o) {
        if (!(o instanceof short[]) || ((short[]) o).length != dimension) {
            return -1;
        }
        short[] array = (short[]) o;
    ...
    public short[][] getRange(int startIndex, int endIndex) {
    ...
    private void readVectorBatch(int startIndex, int count, short[][] dest, int destOffset) throws IOException {
        ...
        for (int i = 0; i < count; i++) {
            ...
            dest[destOffset + i] = new short[dimension];
            buffer.asShortBuffer().get(dest[destOffset + i]);
    ...
    public List<short[]> subList(int fromIndex, int toIndex) {
        ...
        return new SublistSvecReader(this, fromIndex, toIndex);
    }
    ...
    /// A view of a sublist of an SvecReader.
    private static class SublistSvecReader extends ImmutableSizedReader<short[]> {
        private final UniformSvecReader parent;
        private final int offset;
        private final int size;

        public SublistSvecReader(UniformSvecReader parent, int fromIndex, int toIndex) {
            this.parent = parent;
        ...
        @Override
        public int indexOf(Object o) {
            if (!(o instanceof short[]) || ((short[]) o).length != parent.getDimension()) {
                return -1;
            }
            short[] array = (short[]) o;
