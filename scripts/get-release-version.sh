...
then
 printf "The revision '%s' matches the format, continuing\n" ${REVISION} 1>&2
 set -- "${BASH_REMATCH[@]}"
 VERSION_STRING="${@:2:3}"
else
...
export TAG=$(git describe --exact-match --tags 2>/dev/null)
printf "TAG:%s\n" ${TAG} 1>&2
if [[ TAG =~ "^$" ]]
then
 printf "This ref has not been tagged with a valid release tag.\n" 1>&2
 exit 4
fi
