...
/**
 * Truncated normal sampler using proper inverse transform method.
 *
 * <h2>Purpose</h2>
 *
 ...
 * }</pre>
 *
 * @see InverseNormalCDF
 */
public final class TruncatedNormalSampler {
    ...
    /**
     * Creates a truncated normal sampler with specified bounds.
     *
     * @param mean the mean (μ) of the underlying normal distribution
     * @param stdDev the standard deviation (σ) of the underlying normal distribution
     ...
     */
    public TruncatedNormalSampler(double mean, double stdDev, double lower, double upper) {
        if (stdDev <= 0) {
        ...
        this.cdfAtLower = NormalCDF.standardNormalCDF(a);
        double cdfAtUpper = NormalCDF.standardNormalCDF(b);
     ...
     * @return a sample from the truncated normal, guaranteed to be in [lower, upper]
     */
    public double sample(double u) {
        ...
        // Step 5: Invert to get sample
        // x = μ + σ · Φ⁻¹(u')
        return mean + stdDev * InverseNormalCDF.standardNormalQuantile(uPrime);
    }

    /**
     * Returns the mean of the underlying (non-truncated) normal distribution.
     * @return the mean (μ)
     */
    public double mean() {
        return mean;
    ...
    /**
     * Returns the standard deviation of the underlying (non-truncated) normal distribution.
     ...
     */
    public static TruncatedNormalSampler unitBounded(double mean, double stdDev) {
        return new TruncatedNormalSampler(mean, stdDev, -1.0, 1.0);
    }
     ...
     */
    public static TruncatedNormalSampler zeroOneBounded(double mean, double stdDev) {
        return new TruncatedNormalSampler(mean, stdDev, 0.0, 1.0);
    }
}
