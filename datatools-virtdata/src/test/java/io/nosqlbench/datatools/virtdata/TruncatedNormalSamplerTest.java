...
import io.nosqlbench.datatools.virtdata.sampling.TruncatedNormalSampler;
...
/**
 * Tests for {@link TruncatedNormalSampler} verifying proper truncated normal inverse transform.
 */
class TruncatedNormalSamplerTest {

    @Test
    void testOutputBounds() {
        // Standard normal truncated to [-1, 1]
        TruncatedNormalSampler sampler = new TruncatedNormalSampler(0.0, 1.0, -1.0, 1.0);
    ...
    void testAsymmetricBounds() {
        ...
        TruncatedNormalSampler sampler = new TruncatedNormalSampler(0.0, 1.0, 0.0, 1.0);
    ...
    void testWideTruncation() {
        ...
        TruncatedNormalSampler sampler = new TruncatedNormalSampler(0.0, 1.0, -3.0, 3.0);

        double mass = sampler.probabilityMass();
        assertTrue(mass > 0.99, "Probability mass should be >99% for [-3sigma, 3sigma]");
    ...
    void testInvalidBounds() {
        assertThrows(IllegalArgumentException.class,
            () -> new TruncatedNormalSampler(0.0, 1.0, 1.0, -1.0),
            "Should reject lower >= upper");

        assertThrows(IllegalArgumentException.class,
            () -> new TruncatedNormalSampler(0.0, 1.0, 0.0, 0.0),
            "Should reject lower == upper");
