...
/**
 * Fits a Gaussian (normal) distribution to observed data.
 *
 * <h2>Algorithm</h2>
 *
 * <p>Uses maximum likelihood estimation (MLE), which for a Gaussian
 ...
 * (bounded within a finite range) and produce a truncated Gaussian model.
 ...
 */
public final class GaussianModelFitter implements ComponentModelFitter {

    private final boolean detectTruncation;
    private final double truncationThreshold;

    /**
     * Creates a Gaussian model fitter with default settings.
     */
    public GaussianModelFitter() {
        this(true, 0.01);
    }

    /**
     * Creates a Gaussian model fitter with specified settings.
     *
     ...
     */
    public GaussianModelFitter(boolean detectTruncation, double truncationThreshold) {
    ...
    public FitResult fit(DimensionStatistics stats, float[] values) {
        ...
        GaussianComponentModel model;

        if (detectTruncation && detectsTruncation(stats, values)) {
            // Create truncated Gaussian model
            double lower = stats.min();
            double upper = stats.max();
            model = new GaussianComponentModel(mean, stdDev, lower, upper);
        } else {
            // Create unbounded Gaussian model
            model = new GaussianComponentModel(mean, stdDev);
        }
    ...
    public String getModelType() {
        return GaussianComponentModel.MODEL_TYPE;
    }
    ...
    private double computeAndersonDarling(float[] values, double mean, double stdDev) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            double cdfZ = GaussianCDF.standardNormalCDF(z);

            // Clamp to avoid log(0) or log(1)
            cdfZ = Math.max(1e-10, Math.min(1 - 1e-10, cdfZ));

            double weight = 2.0 * (i + 1) - 1;
            sum += weight * (Math.log(cdfZ) + Math.log(1 - GaussianCDF.standardNormalCDF(
