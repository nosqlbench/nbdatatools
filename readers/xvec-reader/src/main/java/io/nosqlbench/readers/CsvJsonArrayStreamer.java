...
public class CsvJsonArrayStreamer implements Streamer<float[]> {
     ...
     *
     * @param filePath The path to the CSV file containing JSON arrays
     */
    ...
    private boolean isLikelyHeaderField(String field) {
        ...
        if (field.length() < 20 && !field.contains(" ") && 
                !field.matches(".*\\d{2,}.*")) { // No sequences of digits
            // Headers often use names like id, name, label, vector, etc.
            String lowerField = field.toLowerCase();
            return lowerField.matches("^[a-z_]+$") ||
                   lowerField.equals("id") ||
                   lowerField.contains("name") ||
                   lowerField.contains("label") ||
                   lowerField.contains("vector") ||
