...
public class CsvJsonArrayStreamer implements Streamer<float[]> {
     ...
     */
    public CsvJsonArrayStreamer(Path filePath) {
    ...
    private void initialize() {
        try (BufferedReader reader = new BufferedReader(new FileReader(filePath.toFile()))) {
            ...
            for (String field : firstLineFields) {
                if (JSON_ARRAY_PATTERN.matcher(field.trim()).matches()
                    && isJsonNumberArray(field.trim()))
                {
            ...
            // If both have arrays or neither has arrays, we need more analysis
                ...
                boolean structuralDifference = detectStructuralDifference(firstLineFields, secondLineFields);
    ...
    private boolean isLikelyHeaderField(String field) {
        ...
        if (field.length() < 20 && !field.contains(" ") && !field.matches(".*\\d{2,}.*")) { // No sequences of digits
            // Headers often use names like id, name, label, vector, etc.
            String lowerField = field.toLowerCase();
            return lowerField.matches("^[a-z_]+$") || lowerField.equals("id") || lowerField.contains("name") || lowerField.contains("label")
                   || lowerField.contains("vector") || lowerField.contains("desc");
        }
        return false;
    }
