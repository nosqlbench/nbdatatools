...
/// later.
/// - Where it makes sense, fvec-extract should be made concurrent
...
///
/// ## IvecExtract (ivec-extract)
///
/// ivec-extract extracts indices from an ivec file using a range specification and writes them to an output ivec file.
/// - It uses random access interfaces through VectorFileIO helper methods for reading input files.
/// - It does basic range checking by determining the record size and then extrapolating the
/// number of records based on the file size.
/// - It provides a progress indicator on stdout, using jline and nice layout
/// - It allows the user to specify which indices will be used by providing a range.
...
/// - The user can provide an advisory limit on the amount of buffer memory to use total
/// - The range values are checked to make sure they are valid, as in positive and in bounds with
/// respect to the size of the ivec file.
///
